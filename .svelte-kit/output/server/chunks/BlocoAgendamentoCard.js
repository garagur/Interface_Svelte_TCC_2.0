...
function BlocoAgendamentoCard($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (ag.sala_nome) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="ag-info svelte-qzbw3r"><span class="material-symbols-outlined ag-icon svelte-qzbw3r">meeting_room</span> <span class="ag-label svelte-qzbw3r">${escape_html(ag.sala_nome)}</span></div>`);
		} else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--> `);
		if (ag.usuario_nome) {
			$$renderer.push("<!--[0-->");
