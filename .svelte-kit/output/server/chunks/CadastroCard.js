...
function CadastroCard($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<!--]--> <button type="submit" class="btn-primary"${attr("disabled", carregando, true)}><span class="material-symbols-outlined">save</span> ${escape_html(carregando ? "Salvando..." : editando ? "Atualizar" : "Salvar")}</button></div></form></div> <div class="card table-card"><div class="table-header-title"><div class="title-left"><span class="material-symbols-outlined text-blue">${escape_html(iconeTabela)}</span> <h3>${escape_html(tituloTabela)}</h3></div> <div class="badge">${escape_html(totalRegistros)} registros</div></div> `);
		if (mostrarPesquisa) {
			$$renderer.push("<!--[0-->");
